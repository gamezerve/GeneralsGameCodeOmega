...
void INI::parseTerrainRoadDefinition( INI* ini )
{
	...
	if( road )
	{
		...
		REBORN_LOG(
			"INI_INVALID_DATA: Terrain entry '%s' is already defined and cannot be redefined as a road. ExistingIsBridge=%d, INIFile='%s', INILine=%d.",
			road->getName().str(),
			static_cast<int>(road->isBridge()),
			ini->getFilename().str(),
			ini->getLineNum());
